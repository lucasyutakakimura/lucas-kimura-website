(()=>{
  ...
  function installImageRefinement(){
  ...
  async function apply(){
    installImageRefinement();

    let heroUrl=null;
    let bleachingUrl=null;

    try{
      heroUrl=await assemble(sources.hero);
    }catch(error){
      console.warn('High-resolution hero could not be assembled; fallback remains active.',error);
    }

    try{
      bleachingUrl=await assemble(sources.bleaching);
    }catch(error){
      console.warn('High-resolution bleaching image could not be assembled; fallback remains active.',error);
    }

    if(heroUrl){
      document.querySelectorAll('.hero>img,.stories article:nth-child(2) img,.field-grid a:nth-child(1) img').forEach(img=>{
        img.src=heroUrl;
    ...
    if(bleachingUrl){
      document.querySelectorAll('.stories article:nth-child(3) img,.field-grid a:nth-child(3) img').forEach(img=>{
